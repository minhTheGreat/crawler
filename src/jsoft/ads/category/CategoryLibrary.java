...
public class CategoryLibrary {
            ...
            for (CategoryObject item : items) {
                if (NO == 0){
                    ...
                    if(item.isSource_enable()){
                        ...
                        tmp += "<td><div class=\"alert alert-danger\"><a href='enable?id="+item.getCategory_source_id()+"' class=\"cate-enable alert-link\">Không s\u1EED d\u1EE5ng</a></div></td>";
                    }
                    tmp += "<td>";
                }
    ...
    public static String viewCategorys(ArrayList<CategoryObject> items) {
        ...
        for (CategoryObject item : items) {
            ...
            if(item.isCategory_enable() && item.isSource_enable()){
                ...
                tmp += "<td><div class=\"alert alert-warning\"><a href='enable?id="+item.getCategory_id()+"' class=\"cate-enable alert-link\">Ngu\u1ED3n không \u0111\u01B0\u1EE3c s\u1EED d\u1EE5ng</a></div></td>";
            } else {
                tmp += "<td><div class=\"alert alert-danger\"><a href='enable?id="+item.getCategory_id()+"' class=\"cate-enable alert-link\">Không s\u1EED d\u1EE5ng</a></div></td>";
            }
    ...
    public static String viewCategoryEdit(CategoryObject category){
        ...
        tmp += "<div class=\"card\">";
        tmp += "<div class=\"card-header\"><strong>C\u1EADp nh\u1EADt chuyên m\u1EE5c</strong></div>";
        tmp += "<div class=\"card-body card-block\">";

        tmp += "<form action='edit?id="+category.getCategory_id()+"' method='POST'>";

        tmp += "<div class=\"form-group\">";
        tmp += "<label for=\"txtName\" class=\" form-control-label\">Tên chuyên m\u1EE5c</label>";
        tmp += "<input type=\"text\" name=\"txtName\" id=\"txtName\" placeholder=\"Tên chuyên m\u1EE5c\" class=\"form-control\" value=\""+category.getCategory_name()+"\" disabled=\"disabled\">";
        tmp += "</div>";

        tmp += "<div class=\"form-group\">";
        tmp += "<label for=\"txtLink\" class=\" form-control-label\">\u0110\u01B0\u1EDDng d\u1EABn chuyên m\u1EE5c</label>";
        tmp += "<input type=\"text\" name=\"txtLink\" id=\"txtLink\" placeholder=\"\u0110\u01B0\u1EDDng d\u1EABn chuyên m\u1EE5c\" class=\"form-control\" value=\""+category.getCategory_link()+"\">";
        tmp += "</div>";
